...
function validateEventFields($eventFields) {
	...
	//
	$distances = array (
		"warmup_distance" => new NumberRange(MIN_WARMUP_DISTANCE,MAX_WARMUP_DISTANCE),
		"run_distance"=> new NumberRange(MIN_RUN_DISTANCE,MAX_RUN_DISTANCE),          
		"cooldown_distance" => new NumberRange(MIN_COOLDOWN_DISTANCE,MAX_COOLDOWN_DISTANCE)
	);
	$keys = array_keys($distances);
	foreach ($keys as $key) {
		$validationResult = validateRealInRange($eventFields-> {
			$key},$distances[$key] );
	...
	//
	$times = array (
		"warmup_time"=> new NumberRange(MIN_WARMUP_TIME,MAX_WARMUP_TIME),
		"run_time"=> new NumberRange(MIN_RUN_TIME,MAX_RUN_TIME),
		"cooldown_time"=> new NumberRange(MIN_COOLDOWN_TIME, MAX_COOLDOWN_TIME)
	);
	
	$keys = array_keys($times);
	foreach ($keys as $key) {
		$validationResult = validateIntInRange($eventFields-> {
			$key }, $times[$key]);
		if (!$validationResult->isValid()) {
			return $validationResult;
		}
